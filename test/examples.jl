@testset "README examples" begin
    ...
    @testset "Second and third examples" begin
        ...
        # Lets create a simple linear layer
        mutable struct SimpleLayer{T}
            W::Matrix{T}
        end
        # Normally ones uses something like randn here, but this makes output below easier on the eyes
        SimpleLayer(nin, nout) = SimpleLayer(ones(Int, nout, nin))
        (l::SimpleLayer)(x) = l.W * x

        # NaiveNASlib needs to know what SimpleLayer considers its output size and input size
        ...
        NaiveNASlib.nin(l::SimpleLayer) = [size(l.W, 2)]
        NaiveNASlib.nout(l::SimpleLayer) = size(l.W, 1)

        # We also need to tell NaiveNASlib how to change the size of SimpleLayer
        # The Δsize function will receive indices to keep from existing weights as well as where to insert new indices
        function NaiveNASlib.Δsize(l::SimpleLayer, newins::AbstractVector, newouts::AbstractVector)
            # newins is a vector of vectors as vertices may have more than one input, but SimpleLayer has only one
        ...
        layer(in, outsize) = absorbvertex(SimpleLayer(nout(in), outsize), in)
        ...
        # This is just because we used nout(in) as the input size when creating SimpleLayer above
        ...
        split = layer(start, nout(invertex) ÷ 3);
        ...
        # Now we have a somewhat complex set of size relations to keep track of as
        #  1. start and joined must have the same output size
        #  2. joined will always have 3 times the outputsize of split
        ...
        # Since out must have the same size as joined, and joined must be three times the size of split
        # By default, NaiveNASlib warns when this happens and then tries to make the closest possible change
        # If we don't want the warning, we can tell NaiveNASlib that we just want the closest possible change right away
        ...
        # parselect will by default insert zeros when size increases. 
        # This helps the graph maintain the same function after mutation
        # In this case we changed the size of the output layer so we don't have the exact same funcion though
        ...
        # Supply a function for telling the value of each neuron
        ...
        # A common appoach when doing structured pruning is to prefer neurons with high magnitude.
        # Here is how to set that as the default for SimpleLayer
        import Statistics: mean
        NaiveNASlib.default_outvalue(t, l::SimpleLayer) = mean(abs, l.W, dims=2)
        ...
        # This can be done by just not putting any size requirements and let the value function deal with it
        ...
        @testset "Add layers example" begin
            ...
            @test insert!(invertex, vertex -> layer(vertex, nout(vertex))) # True if success
        ...
        @testset "Pruning example" begin
            ...
            function vertexandlayer(in, outsize)
                ...
                l = SimpleLayer(collect(reshape(1:nparam, :, nout(in))))
            ...
            # v2 lost its middle row (the one with lowest value)
            @test l2.W ==
            [ 1 4 7 10 ;
              3 6 9 12 ]
        ...
        @testset "Advanced usage" begin
            ...
            @testset "Traits" begin
                ...
                named = absorbvertex(SimpleLayer(2, 3), inputvertex("in", 2), traitdecoration = t -> NamedTrait(t, "named layer"))
                @test name(named) == "named layer"

                # Speaking of logging...
                layer1 = absorbvertex(SimpleLayer(2, 3), inputvertex("in", 2), traitdecoration = t -> SizeChangeLogger(NamedTrait(t, "layer1")))

                # What info is shown can be controlled by supplying an extra argument to SizeChangeLogger
                nameonly = NameInfoStr()
                layer2 = absorbvertex(SimpleLayer(nout(layer1), 4), layer1, traitdecoration = t -> SizeChangeLogger(nameonly, NamedTrait(t, "layer2")))
                ...
                (:info, "Change nin of layer2 by [1, 2, 3, -1]"), # Note: less verbose compared to layer1 due to NameInfoStr
                 ...
                 (:info, "Change nout of layer1 + layer2, inputs=[layer1, layer2], outputs=[], nin=[5, 5], nout=[5], SizeInvariant() by [1, 2, 3, 4, -1]"),
                  ...
                  painlayer = absorbvertex(SimpleLayer(2, 3), inputvertex("in", 2), traitdecoration = PainfulTrait)
                  ...
                  smoothlayer = absorbvertex(SimpleLayer(2, 3), inputvertex("in", 2), traitdecoration = SmoothSailingTrait)
